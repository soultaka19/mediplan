...
export class AppointmentBookingPage {
  ...
  load(): void {
    ...
    forkJoin({
      doctors: this.userService.listUsers().pipe(catchError(() => of([] as PublicUser[]))),
      availabilities: this.availabilityService
        .listAvailabilities()
        .pipe(catchError(() => of([] as Availability[]))),
    })
  ...
  submit(): void {
    ...
    this.bookingService
      ...
      .subscribe({
        next: () => {
          this.notifications.success('Rendez-vous réservé.');
          void this.router.navigate(['/clinic-flow/today']);
        },
        error: (err: unknown) => this.error.set(authErrorMessage(err)),
      });
  }
